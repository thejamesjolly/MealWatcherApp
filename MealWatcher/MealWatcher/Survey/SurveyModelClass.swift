...
final class SurveyItem : Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        case .timeInputQuestion:
            self.question = try container.decode(TimeInputQuestion.self, forKey: .question)
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .timeInputQuestion:
            try container.encode(self.question as! TimeInputQuestion, forKey: .question)
        }
...
class TimeInputQuestion : ObservableObject, SurveyQuestion {
    ...
    // Info
    var emailAddress : String = ""
    var timeOnWheels : Date = Date.now //Default to current time
    var feedback : String = Date.now.formatted()
